OC.L10N.register(
    "facerecognition",
    {
    "The analysis is finished" : "Analiza zakończona",
    "_1 image was analyzed_::_{totalImages} images were analyzed_" : ["1 obraz został przeanalizowany","obrazy zostały przeanalizowane","obrazów zostało przeanalizowanych","{totalImages} obrazów zostało przeanalizowanych"],
    "Analyzing images" : "Analizuje obrazy",
    "_1 image detected_::_{totalImages} images detected_" : ["obraz rozpoznany","obrazy rozpoznane","obrazów rozpoznanych","{totalImages} obrazów rozpoznanych"],
    "_1 image in queue_::_{queuedImages} images in queue_" : ["obraz w kolejce","obrazy w kolejce","obrazów w kolejce","{queuedImages} obrazów w kolejce"],
    "Ends approximately {estimatedFinalize}" : "Koniec za około {estimatedFinalize}",
    "The analysis is not started yet" : "Analiza jeszcze się nie rozpoczęła",
    "The changes were saved. It will be taken into account in the next analysis." : "Zmiany zostały zapisane. Będą wzięte pod uwagę przy następnej analizie.",
    "The change could not be applied." : "Zmiana nie mogła zostać zapisana.",
    "Persons" : "Ludzie",
    "Looking for faces in this image…" : "Szukam twarz na tym obrazie...",
    "Rename" : "Zmień nazwę",
    "Open <a target=\"_blank\" href=\"{settingsLink}\">settings ↗</a> to enable it." : "Otwórz <a target=\"_blank\" href=\"{settingsLink}\">ustawienia ↗</a> aby włączyć.",
    "Facial recognition is disabled" : "Rozpoznawanie twarzy wyłączone",
    "No people found" : "Nie znaleziono ludzi",
    "This image is not yet analyzed" : "Obraz jeszcze nie przeanalizowany",
    "Please, be patient" : "Proszę czekać",
    "Rename Person" : "Zmień imię",
    "Please enter a name to rename the person" : "Proszę wprowadź imię aby zmienić",
    "Cancel" : "Anuluj",
    "There was an error trying to show your friends" : "Wystąpił błąd przy wyświetlaniu twoich znajomych",
    "The analysis is enabled, please be patient, you will soon see your friends here." : "Analiza jest włączona, proszę czekać, niedługo będzie można zobaczyć tu swoich znajomych.",
    "The analysis is disabled. Soon all the information found for facial recognition will be removed." : "Analiza jest wyłączona. Niedługo informacje o rozpoznawaniu twarzy zostaną usunięte.",
    "Face Recognition" : "Rozpoznawanie twarzy",
    "Here you can see photos of your friends that are recognized" : "Tu możesz zobaczyć zdjęcia swoich znajomych które zostały rozpoznane.",
    "Analyze my images and group my loved ones with similar faces" : "Przeanalizuj moje obrazy i pogrupuj moich bliskich z podobnymi twarzami.",
    "Looking for your recognized friends" : "Szukam twoich rozpoznanych znajomych",
    "Your friends have not been recognized yet" : "Twoi znajomi nie zostali jeszcze rozpoznani",
    "The analysis is disabled" : "Analiza jest wyłączona",
    "Enable it to find your loved ones" : "Włącz, aby znaleźć twoich bliskich",
    "You will lose all the information analyzed, and if you re-enable it, you will start from scratch." : "Stracisz cala informacje o analizie i jeżeli włączysz to ponownie zaczniesz od zera.",
    "Do you want to deactivate the grouping by faces?" : "Czy chciałbyś wyłączyć grupowanie według twarzy?",
    "There was an error when trying to find photos of your friend" : "Wystąpił błąd przy znajdowaniu zdjęć twojego znajomego.",
    "There was an error renaming this person" : "Wystąpił błąd przy zmienianiu nazwy tej osoby",
    "Recommended memory for analysis is at least 1GB of RAM." : "Rekomendowana ilość pamięci do analizy to minimum 1GB RAMu",
    "We are not recommending using more than 4GB of RAM memory, as we see little to no benefit." : "Nie rekomendujemy używania więcej niż 4GB RAMu gdyż nie widzieliśmy znacznych różnic.",
    "According to your system you can not use more than %s GB of RAM" : "Według twojego systemu nie możesz użyć więcej niż %s GB RAMu",
    "The format seems to be incorrect." : "Format jest niepoprawny.",
    "A face recognition app" : "Aplikacja do rozpoznawania twarzy",
    "FaceRecognition is a Nextcloud application with a goal of recognizing, analyzing and aggregating face data in users images, and providing additional functionalities on top of these information, all with built-in privacy of Nextcloud" : "FaceRecognition jest aplikacja na Nextcloud, która ma na celu rozpoznawanie, analizowanie i agregowanie informacji na temat twarzy w zdjęciach użytkowników i zapewnić dodatkowe funkcje używając tych informacji, wszystko z myślą o prywatności która jest wbudowana w Nextcloud.",
    "Open Documentation" : "Otwarta dokumentacja",
    "Sensitivity" : "Czułość",
    "The sensitivity determines how different the faces can be to continue to be considered as the same person." : "Czułość determinuje jak podobne muszą być twarze do siebie żeby były uznane za tą samą osobę.",
    "More sensitivity, more groups" : "Większa czułość, więcej grup",
    "Less sensitivity, less groups" : "Mniejsza czulosc, mniej grup",
    "Restore" : "Odtwórz",
    "Save" : "Zapisz",
    "Minimum confidence" : "Minimalna pewność",
    "The minimum confidence determines how reliable must be a face detection to try to group it. Blurred or misaligned faces would have a confidence closer to 0.0, and the best images close to 1.0." : "Minimalna pewność determinuje, jak dokładnie twarz musi zostać rozpoznana żeby przydzielić ją do grupy. Rozmazane albo krzywo ustawione twarze będą miały pewność rozpoznania bliską 0.0 gdzie najlepsze zdjęcia będą miały pewność bliską 1.0.",
    "Less minimum confidence" : "Mniejsza minimalna pewność",
    "Highest minimum confidence" : "Najwieksza minimalna pewność",
    "Memory limits" : "Limit pamięci",
    "Assigning more RAM can improve the results, but the analysis will be slower. Limiting its use you will get results faster, but for example you can lose the discovery of smaller faces in your images." : "Przydzielenie większej ilości RAMu może zwiększyć jakość rezultatów, ale analiza zajmie więcej czasu. Mniejsza ilość da szybsze rezultaty, ale, na przykład, możesz nie uzyskać rozpoznania mniejszych twarz w twoich zdjęciach.",
    "Use suffix as 2048 MB or 2G" : "Użyj sufiksu takiego jak 2048MB albo 2G",
    "Additional settings" : "Ustawienia dodatkowe",
    "Show persons with a only 1 face found" : "Pokaż ludzi ze znaleziona tylko 1 twarzą",
    "Configuration information" : "Informacje o konfiguracji",
    "Current status" : "Aktualny status",
    "Stopped" : "Zatrzymane"
},
"nplurals=4; plural=(n==1 ? 0 : (n%10>=2 && n%10<=4) && (n%100<12 || n%100>14) ? 1 : n!=1 && (n%10>=0 && n%10<=1) || (n%10>=5 && n%10<=9) || (n%100>=12 && n%100<=14) ? 2 : 3);");
