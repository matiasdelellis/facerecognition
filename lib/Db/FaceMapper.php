...
class FaceMapper extends QBMapper
{
    ...
    public function find(int $faceId, string $userId): ?Face{
        ...
        try {
            ...
            $this->logger->debug('FaceMapper -- find -- Found face ID ' . $faceId . ' for user ' . $userId);
            return $entity;
        } catch (DoesNotExistException $e) {
            $this->logger->info('FaceMapper -- find -- No face found for user ' . $userId . ', face ID ' . $faceId);
            return null;
        }
    }
    ...
    public function findDescriptorsBathed(array $faceIds): array{
        ...
        for ($i = 0; $i < count($faceIds); $i= $i+1000)
        {
            ...
            $this->logger->debug('FaceMapper -- findDescriptorsBathed -- processed batch ' . ($i/1000 + 1) . ' with ' . count($faces) . ' faces');
        }

        $this->logger->debug('FaceMapper -- findDescriptorsBathed -- Retrieved total ' . count($descriptors) . ' descriptors for ' . count($faceIds) . ' requested faces');
        return $descriptors;
    }
    ...
    public function findFromFile(string $userId, int $modelId, int $fileId): array{
        ...
        $faces = $this->findEntities($qb);
        $this->logger->debug('FaceMapper -- findFromFile -- Found ' . count($faces) . ' faces for user ' . $userId . ', model ' . $modelId . ', file ' . $fileId);
        return $faces;
    }
    ...
    public function countFaces(string $userId, int $model, bool $onlyWithoutPersons = false): int{
        ...
        $count = (int)$data[0];
        $this->logger->debug('FaceMapper -- countFaces -- Counting faces for user ' . $userId . ', model ' . $model . ', onlyWithoutPersons: ' . ($onlyWithoutPersons ? 'true' : 'false') . ' RETURNED COUNT: ' . $count);
        return $count;
    }
    ...
    public function getOldestCreatedFaceWithoutPerson(string $userId, int $model): ?Face{
        ...
        try {
            ...
            $this->logger->debug('FaceMapper -- getOldestCreatedFaceWithoutPerson -- Found face ID ' . $face->getId() . ' for user ' . $userId . ', model ' . $model);
            return $face;
        } catch (DoesNotExistException $e) {
            $this->logger->info('FaceMapper -- getOldestCreatedFaceWithoutPerson -- No face without person found for user ' . $userId . ', model ' . $model);
            return null;
        }
    }
    ...
    public function getFaces(string $userId, int $model): array{
        ...
        $qb->select('f.id', 'cf.cluster_id as person', 'f.image_id as image', 'x', 'y', 'width', 'height', 'landmarks', 'descriptor', 'confidence', 'creation_time', $qb->createFunction("COALESCE(cf.is_groupable, 'true') as is_groupable"))
            ...
            ->setParameter('model', $model);
        $faces = $this->findEntities($qb);
        $this->logger->debug('FaceMapper -- getFaces -- Found ' . count($faces) . ' faces for user ' . $userId . ', model ' . $model . ' (used in tests)');
        return $faces;
    }
    ...
    public function getGroupableFaces(string $userId, int $model, int $minSize, float $minConfidence): array{
        ...
        $faces = $this->findEntities($qb);
        $this->logger->debug('FaceMapper -- getGroupableFaces -- Found ' . count($faces) . ' groupable faces for user ' . $userId . ', model ' . $model . ', minSize ' . $minSize . ', minConfidence ' . $minConfidence);
        return $faces;
    }
    ...
    public function getNonGroupableFaces(string $userId, int $model, int $minSize, float $minConfidence): array{
        ...
        $faces = $this->findEntities($qb);
        $this->logger->debug('FaceMapper -- getNonGroupableFaces -- Found ' . count($faces) . ' non-groupable faces for user ' . $userId . ', model ' . $model . ', minSize ' . $minSize . ', minConfidence ' . $minConfidence);
        return $faces;
    }
    ...
    public function findFromCluster(string $userId, int $clusterId, int $model, ?int $limit = null, ?int $offset = null): array{
        ...
        $faces = $this->findEntities($qb);
        $this->logger->debug('FaceMapper -- findFromCluster -- Found ' . count($faces) . ' faces for user ' . $userId . ', cluster ' . $clusterId . ', model ' . $model);
        return $faces;
    }
    ...
    public function findFromPerson(string $userId, string $personId, int $model, ?int $limit = null, ?int $offset = null): array{
        ...
        $faces = $this->findEntities($qb);
        $this->logger->debug('FaceMapper -- findFromPerson -- Found ' . count($faces) . ' faces for user ' . $userId . ', person ' . $personId . ', model ' . $model . ', offset ' . ($offset ?? 'NULL') . ', limit ' . ($limit ?? 'NULL'));
        return $faces;
    }
    ...
    public function findByImage(int $imageId): array{
        ...
        $faces = $this->findEntities($qb);
        $this->logger->debug('FaceMapper -- findByImage -- Found ' . count($faces) . ' faces in image ID ' . $imageId);
        return $faces;
    }
    ...
    public function removeFromImage(int $imageId, ?IDBConnection $db = null): void{
        ...
        $this->logger->info('FaceMapper -- removeFromImage -- Removed all faces from image ID ' . $imageId);
    }
    ...
    public function deleteUserModel(string $userId, $modelId): void{
        ...
        $this->logger->info('FaceMapper -- deleteUserModel -- Deleted faces for user ' . $userId . ' and model ' . $modelId);
    }
    ...
    public function unsetPersonsRelationForUser(string $userId, int $model): void{
        ...
        $this->logger->info('FaceMapper -- unsetPersonsRelationForUser -- Unset person relations for user ' . $userId . ' and model ' . $model);
    }
    ...
    public function insertFace(Face $face, ?IDBConnection $db = null): Face{
        ...
        if ($face->person !== null) {
            ...
            $insertPersonFaceConnection->insert('facerecog_cluster_faces')
                ...
                ->executeStatement();
            $this->logger->info('FaceMapper -- insertFace -- Inserted face ID ' . $face->getId() . ' for image ' . $face->image . ' and associated to cluster ' . $face->person);
        } else {
            $this->logger->info('FaceMapper -- insertFace -- Inserted face ID ' . $face->getId() . ' for image ' . $face->image);
        }
        return $face;
    }
}
