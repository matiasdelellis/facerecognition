    ...
    public function getNonGroupableFaces(string $userId, int $model, int $minSize, float $minConfidence): array {
        ...
        try {
            ...
            $this->logger->debug(sprintf(
                'FaceMapper -- getNonGroupableFaces -- Found %d non-groupable faces for user %s, model %d (minSize=%d, minConfidence=%.2f)',
                count($faces),
                $userId,
                $model,
                $minSize,
            ...
            $this->logger->error('FaceMapper -- getNonGroupableFaces -- Error: ' . $e->getMessage(), [
                'user' => $userId,
                'model' => $model,
                'minSize' => $minSize,
                'minConfidence' => $minConfidence,
